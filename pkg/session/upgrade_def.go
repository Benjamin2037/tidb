...
const (
	...
	// version254
	// Add mysql.tidb_import_slo_guard table.
	version254 = 254

	// version255
	// Add config JSON column to mysql.tidb_import_slo_guard.
	version255 = 255
)
...
func upgradeToVer254(s sessionapi.Session, _ int64) {
	mustExecute(s, metadef.CreateTiDBImportSLOGuardTable)
	mustExecute(s, "INSERT IGNORE INTO mysql.tidb_import_slo_guard (job_id, enable, pause_threshold, slow_apply_rate_limit_mb_per_sec) VALUES (0, 0, '10ms', 64)")
}

func upgradeToVer255(s sessionapi.Session, _ int64) {
	doReentrantDDL(s, "ALTER TABLE mysql.tidb_import_slo_guard ADD COLUMN `config` JSON DEFAULT NULL AFTER `job_id`", infoschema.ErrColumnExists)
}
