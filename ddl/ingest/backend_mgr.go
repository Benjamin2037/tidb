...
func (m *backendCtxManager) Register(ctx context.Context, unique bool, jobID int64, indexNum int, _ mysql.SQLMode) (*BackendContext, error) {
	...
	if !exist {
		...
		cfg, err := genConfig(m.memRoot, jobID, unique, int64(indexNum))
		if err != nil {
